...
const configDir = path.join(os.homedir(), '.projj');
...
class Command extends BaseCommand {
  ...
  async addRepo(repo, cacheKey) {
    ...
    await this.runScript(`git clone -q ${repo} ${targetPath}`, {
      env,
    });
  ...
  config.base = config.base.map(baseDir => {
    switch (baseDir[0]) {
      ...
      case '/':
        return baseDir;
      default:
        return process.platform === 'win32' ? baseDir : path.join(process.cwd(), baseDir);
    }
  });
  return config;
}
